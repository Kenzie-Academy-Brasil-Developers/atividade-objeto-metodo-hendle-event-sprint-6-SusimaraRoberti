...
Frame_imagens é um array das imagens
 * target_div is the HTML element within which the image should be appended. 
target_div é o elemento html onde as imagens serão "append"
 * x and y are the coordinates for where to position the image.
X e Y são as coordenadas de onde posicionar a imagem
 */
class ClickToAdvance {
  constructor(frame_images, target_div, x, y) {
  ...
  /* TODO: Implement a prototype method named handleEvent (see Note 1 below).
    ...
    Implemente um método de protótipo chamado handleEvent (veja a Nota 1 abaixo). 
     "handleEvent" deve avançar para o próximo quadro na sequência.
     * 
     * In other words, add 1 to this.frame, and set this.img.src = this.images[this.frame]
     * Em outras palavras, adicione 1 a this.frame e defina this.img.src = this.images[this.frame]
     * 
     * Dont increase this.frame beyond the number of frame_images available.
     * Não aumente this.frame além do número de frame_images disponíveis.
     * 
     ...
     * Depois de implementar o método handleEvent, você pode descomentar o
     * abaixo para adicionar este objeto como o ouvinte do evento de clique para a imagem.
     */
  handleEvent() {
    this.frame += 1;
    if (this.frame > 2) {
...
No nosso caso, este objeto é a instância da nossa classe ClickToAdvance e, como sempre, estamos representando com `this`.
